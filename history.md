...
## Phase 18: Snowpack Data Accuracy & HRRR Model Fix

### What changed

**1. Fixed HRRR model name (critical)**
- The Open-Meteo API renamed the HRRR model identifier from `hrrr` to `ncep_hrrr_conus_15min`.
- Our code was using the old name, causing HRRR requests to fail with HTTP 400 for ALL US resorts.
- `Promise.allSettled` silently swallowed the error, so the app worked but with only GFS + ECMWF data — losing HRRR's high-resolution (3km) short-range (48h) forecast.
- Fixed `modelsForCountry('US')` to return `'ncep_hrrr_conus_15min'`.

**2. Fixed null handling in snow_depth model averaging**
- ECMWF returns `snow_depth` as an array of `null` values (it doesn't provide this variable).
- The averaging code checked `model.snow_depth?.[idx] !== undefined` — but `null !== undefined` is `true` in JavaScript, so `null` values were pushed into the averages array and coerced to `0` in arithmetic, diluting the mean.
- Changed to `!= null` which correctly filters out both `null` and `undefined`.
- Only models with real snow_depth data (GFS, GEM) now contribute to the averaged value.

**3. Snowpack display: single grid-cell value**
- Investigation confirmed that Open-Meteo's `snow_depth` is a grid-cell value from the weather model — the `elevation` parameter does NOT affect it.
- All three elevation bands (base/mid/top) return the same snow_depth from the same grid cell.
- Changed ConditionsSummary snowpack row from per-band columns to a single full-width value (matching the freezing level row pattern).
- ResortPage header stat still shows the value for the selected band (which will be the same across bands after the averaging fix).

### API investigation findings
- `snow_depth` unit confirmed: **meters** (not cm). The `* 100` conversion to cm is correct.
- GFS (`gfs_seamless`): Provides real snow_depth values. Confirmed same value regardless of `elevation` parameter.
- ECMWF (`ecmwf_ifs025`): Returns `snow_depth` with unit "undefined" and all-null values — does not support this variable.
- HRRR (`ncep_hrrr_conus_15min`): Returns snow_depth with null values — does not support this variable, but accepts it without error.
- GEM (`gem_seamless`): Provides real snow_depth values (for Canadian resorts).

### Key files affected
- `src/utils/modelAverage.ts` — Fixed HRRR model name, fixed null handling in `averageHourlyArrays`
- `src/components/ConditionsSummary.tsx` — Snowpack row changed to single full-width value
- `src/utils/__tests__/modelAverage.test.ts` — Updated HRRR test, added 3 new snow_depth tests
- `src/components/__tests__/ConditionsSummary.test.tsx` — Added 2 snowpack rendering tests

### Test impact
- 220 tests across 21 files, all passing (5 new tests added)
